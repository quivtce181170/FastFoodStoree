...
<head>
    ...
    <style>
        ...
        h1 {
            color: #007bff; /* Màu chữ cho tiêu đề Edit Voucher */
            text-align: center;
    ...
    <form method="post" action="<%= request.getContextPath() %>/Quanly/voucher/EditVoucher">
        ...
        <div>
            ...
            <input type="number" id="quantity" name="quantity" required min="1" max="99" 
                   value="<%= request.getAttribute("voucher") != null ? ((Voucher) request.getAttribute("voucher")).getSoLuong(): "" %>">
        </div>
        <div>
        ...
        <div class="button-container">
            <!-- Nút trở về danh sách voucher và nút lưu thay đổi -->
            <button type="button" class="button back-button" onclick="window.location.href = '<%= request.getContextPath() %>/Quanly/voucher/ListVoucher'" id="back">Back</button>
            <button type="submit" class="button edit-button" id="submit" style="margin-left: 10px;">Save Changes</button>
        </div>
    </form>
</body>
</html>
